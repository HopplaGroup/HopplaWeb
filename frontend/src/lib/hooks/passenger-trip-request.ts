...
import type { PreferredTimeSlot, PassengerTripRequestStatus } from '@zenstackhq/runtime/models';

export function useCheckPassengerTripRequest<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; passengerId?: string; from?: string; to?: string; preferredTimeSlot?: PreferredTimeSlot; seatsNeeded?: number; description?: string; status?: PassengerTripRequestStatus }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
